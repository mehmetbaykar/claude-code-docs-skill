...
Declaring `relevance` in `marketplace.json` is not enough on its own. An administrator must allowlist the marketplace in [managed settings](https://code.claude.com/docs/en/managed-settings) before its suggestions appear to users.
...
* [Create and distribute a plugin marketplace](https://code.claude.com/docs/en/plugin-marketplaces): build the marketplace that hosts your plugins
* [Recommend your plugin from your CLI](https://code.claude.com/docs/en/plugin-hints): prompt users from your own CLI instead of from Claude Code's session signals
* [All settings](https://code.claude.com/docs/en/settings-reference#pluginsuggestionmarketplaces): `pluginSuggestionMarketplaces` and `extraKnownMarketplaces`
