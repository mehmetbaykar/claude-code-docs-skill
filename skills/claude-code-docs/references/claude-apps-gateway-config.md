...
* It is never merged when an MDM or file-based source wins and its [`policyHelper`](https://code.claude.com/docs/en/settings#compute-managed-settings-with-a-policy-helper) supplies managed settings.
...
The following keys are honored when any admin source above the user-writable HKCU tier sets them, regardless of which source provides the rest of the policy. When an MDM or file-based source wins and its [`policyHelper`](https://code.claude.com/docs/en/settings#compute-managed-settings-with-a-policy-helper) supplies managed settings, the helper's output is the only source these checks read:
