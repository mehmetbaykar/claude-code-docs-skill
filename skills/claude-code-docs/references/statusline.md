...
* If `disableAllHooks` is set to `true` in your settings, Claude Code runs only a `statusLine` from managed settings, and with no managed `statusLine` the status line is disabled. Remove the setting or set it to `false` to re-enable. See [Hook configuration](https://code.claude.com/docs/en/settings#hook-configuration).
