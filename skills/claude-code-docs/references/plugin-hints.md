...
* `CLAUDE_CODE_CHILD_SESSION`: set only in subprocesses Claude Code itself spawns, such as tool calls, hook commands, and [status line](https://code.claude.com/docs/en/statusline) commands, so the tag does not normally reach a human terminal. A long-lived process that was started inside a session, such as a tmux server, captures the variable, so shells later launched from that process still show the raw tag. Requires Claude Code v2.1.172 or later, so sessions on older versions miss the hint.
...
* **Main interactive session only**: Claude Code shows the prompt only in the terminal session the user is typing into. Claude Code never prompts for a command that a [subagent](https://code.claude.com/docs/en/sub-agents) runs, and never prompts when the user runs Claude Code in [non-interactive mode](https://code.claude.com/docs/en/headless) with the `-p` flag or through the [Agent SDK](https://code.claude.com/docs/en/agent-sdk/overview). Claude Code still strips the hint line from the command output in all of these cases.
* **Telemetry opt-outs**: sessions where analytics are disabled never show hint prompts. This includes sessions with `DISABLE_TELEMETRY` or `CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC` set, and sessions on third-party providers such as Amazon Bedrock or Google Cloud's Agent Platform where the [automatic telemetry opt-out](https://code.claude.com/docs/en/data-usage#default-behaviors-by-api-provider) applies.
...
The hint protocol only takes effect for plugins listed in the official Anthropic marketplace, `claude-plugins-official`. Anthropic curates that marketplace at its discretion, and the in-app submission forms add plugins to the [community marketplace](https://code.claude.com/docs/en/plugins#submit-your-plugin-to-the-community-marketplace) instead, which the hint protocol does not check. If you are working with an Anthropic partner contact, reach out to them to coordinate an official-marketplace listing.
...
* [Environment variables](https://code.claude.com/docs/en/env-vars): full reference for `CLAUDECODE` and related variables
