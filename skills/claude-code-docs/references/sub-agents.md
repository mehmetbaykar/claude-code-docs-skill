...
To see the debug log, run Claude Code with `--debug`. A [plugin subagent](https://code.claude.com/docs/en/plugins-reference#agents) whose frontmatter has no `name` or doesn't parse still loads, under its filename.
