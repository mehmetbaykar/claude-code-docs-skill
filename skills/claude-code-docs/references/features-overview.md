...
* **Agent teams** are independent Claude Code sessions that communicate with each other
    ...
    | **Communication** | Returns a result to the caller. Subagents that Claude named when it spawned them can also [message each other](https://code.claude.com/docs/en/sub-agents#what-loads-at-startup) | Teammates message each other directly                                                                                                         |
    ...
    **Transition point:** If you're running parallel subagents but hitting context limits, agent teams are the natural next step. For separate sessions that pass messages to each other without a team, see [cross-session messaging](https://code.claude.com/docs/en/cross-session-messaging).
