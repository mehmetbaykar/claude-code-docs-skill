...
The picker is the interactive model list that opens when a developer runs `/model` in Claude Code. Each discovered entry is labeled "From gateway" and uses `display_name` when provided. The [`availableModels` managed setting](https://code.claude.com/docs/en/settings#available-settings) bounds what discovery can add.
