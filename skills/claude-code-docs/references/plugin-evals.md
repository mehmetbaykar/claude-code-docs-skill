---
title: "Test plugins with evals"
source: https://code.claude.com/docs/en/plugin-evals
path: /docs/en/plugin-evals
---
...
`claude plugin eval` runs your [plugin](https://code.claude.com/docs/en/plugins) against a suite of test cases and scores the results. Each case is a realistic prompt plus one or more graders. A grader is a pass/fail check on what Claude produced, such as a regex over the reply, whether a particular tool was called, or a rubric that a second model judges the reply against.
...
* A plugin directory with a `plugin.json` or `.claude-plugin/plugin.json` manifest, or a [skills-directory plugin](https://code.claude.com/docs/en/plugins-reference#skills-directory-plugins).
...
For each run of a case, Claude Code starts a fresh, [isolated](#how-runs-are-isolated) [non-interactive session](https://code.claude.com/docs/en/headless) with only your plugin loaded, sends the prompt, and lets Claude work until it finishes or hits the case's turn or time limit. Each grader then checks the final reply, the full transcript, or a file Claude created, and passes or fails.
...
| `name@skills-dir`                                         | The same, for a [skills-directory plugin](https://code.claude.com/docs/en/plugins-reference#skills-directory-plugins)                                                                                                         |
...
* [Create plugins](https://code.claude.com/docs/en/plugins): build the plugin you're testing, and load it with `--plugin-dir` during development
* [Plugins reference](https://code.claude.com/docs/en/plugins-reference#plugin-eval): the `plugin eval` and `plugin eval init` command entries and the manifest's `experimental.evals` key
...
* [Create and distribute a plugin marketplace](https://code.claude.com/docs/en/plugin-marketplaces): publish the plugin once its suite passes
