...
| Optional pairing or walkthroughs        | 0 to 30 minutes  | Reserve this for colleagues who are blocked, and offer the [Quickstart](https://code.claude.com/docs/en/quickstart) link before scheduling time. |
...
| "How do I trust it with my code?"                  | Introduce plan mode: pressing `Shift+Tab` cycles into it, Claude proposes exactly what it intends to change, and nothing is modified until the user approves.                                                                | [Permissions](https://code.claude.com/docs/en/permissions)                          |
...
| "It does not understand our codebase conventions." | Suggest running `/init` to generate a `CLAUDE.md` file, then adding the team's conventions, test commands, and any directories that should be avoided.                                                                       | [Memory](https://code.claude.com/docs/en/memory)                                    |
...
| ------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------ |
| A dedicated channel                        | Create a `#claude-code` channel (or a recurring thread in an existing one), pin the [Quickstart](https://code.claude.com/docs/en/quickstart) link and one strong example, and answer questions publicly so each answer benefits everyone watching.                            | About five minutes to set up, then ambient |
...
Create the channel, pin the [Quickstart](https://code.claude.com/docs/en/quickstart), and post two or three of your own examples with the prompts included.
...
You are the warm introduction rather than the onboarding program. When a colleague moves past "should I try this" into "how do I become effective with it," point them to the [Quickstart](https://code.claude.com/docs/en/quickstart) and [Common workflows](https://code.claude.com/docs/en/common-workflows) pages. They contain short sections covering the features that are useful but difficult to discover on your own.
...
| Teach it your repository         | Run `/init` to generate a `CLAUDE.md` file, then add your conventions, test commands, and any directories that should not be modified. See [Memory](https://code.claude.com/docs/en/memory). |
| Reuse a workflow                 | Save a `SKILL.md` file in `.claude/skills/<name>/` to create a `/name` skill that the entire team can use. See [Skills](https://code.claude.com/docs/en/skills).                             |
| Stay informed during long tasks  | Configure a Stop hook to receive a desktop notification when a long-running task completes. See [Hooks](https://code.claude.com/docs/en/hooks-guide).                                        |
...
Claude Code is updated frequently. Verify version-specific details against the [documentation home page](https://code.claude.com/docs/en/overview) before distributing this material internally.
