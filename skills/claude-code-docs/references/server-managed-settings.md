...
* **Cross-source lock keys**: a small set of keys, such as the sandbox allowlist locks, [listed in the settings reference](https://code.claude.com/docs/en/settings#precedence-within-the-managed-tier). They are honored when any admin-controlled managed source sets them; the user-writable HKCU registry tier is excluded, and when an MDM or file-based source wins and configures a [`policyHelper`](https://code.claude.com/docs/en/settings#compute-managed-settings-with-a-policy-helper), the helper's output is the only source these checks read.
