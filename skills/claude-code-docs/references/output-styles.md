...
For instructions about your project, conventions, or codebase, use [CLAUDE.md](https://code.claude.com/docs/en/memory) instead.
...
* **VS Code extension**: open the [command menu](https://code.claude.com/docs/en/vs-code#use-the-prompt-box) with `/` and select **Output styles** to pick a style, including your custom styles. Claude Code saves your selection to `.claude/settings.local.json`, the same file the terminal menu writes. Requires Claude Code v2.1.257 or later.
...
Output style is part of the system prompt, which Claude Code reads once at session start. Changes take effect after `/clear` or a new session. See [How Claude Code uses prompt caching](https://code.claude.com/docs/en/prompt-caching#changing-output-style) for what an output style change does to the cache.
...
Output styles apply to the main conversation only: a [subagent runs its own system prompt](https://code.claude.com/docs/en/sub-agents#what-loads-at-startup), so styles don't change how subagents respond. A [fork](https://code.claude.com/docs/en/sub-agents#fork-the-current-conversation) is the exception, because it inherits the parent's full system prompt.
