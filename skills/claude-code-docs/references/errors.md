...
The trailing hint names the closest matching alias or model ID. When nothing is close enough, it reads `Run /model to see available models.` instead. In a session that the [Desktop app](https://code.claude.com/docs/en/desktop) starts for you, the no-match hint reads `Switch to a different model.`
...
When the error does appear, the refused system message isn't one Claude Code can remove. That usually means a proxy or [LLM gateway](https://code.claude.com/docs/en/llm-gateway) between Claude Code and the API added a system message of its own or reordered the conversation.
...
You tried to start a [background session](https://code.claude.com/docs/en/agent-view) in a directory that doesn't exist anymore. Claude Code doesn't start the session, and the message names the missing directory:
```text
