...
| `assets-proxy.anthropic.com`   | Application code for [Claude Code on the web](/en/claude-code-on-the-web) and the [Claude Desktop app](/en/desktop). For the web app, `claude.ai` serves only the entry page's HTML; the application's JavaScript and CSS bundles load from this host. Required in the browser or desktop app, not by the CLI itself                                                                                        |
