...
When a timer aborts a stalled stream, Claude Code handles it like any other mid-stream failure: it retries, keeps completed output with an [incomplete-response notice](https://code.claude.com/docs/en/errors#the-response-above-may-be-incomplete), or ends the turn, depending on where the response stood.
...
| `storage.googleapis.com`             | Install counts and plugin metadata shown in `/plugin`. Signed [artifact](https://code.claude.com/docs/en/artifacts) uploads try this host first; publishing falls back to `api.anthropic.com` when it is blocked                                                                                                                                                                                                                        |
