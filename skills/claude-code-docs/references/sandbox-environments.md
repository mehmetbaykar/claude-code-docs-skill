...
The [sandboxed Bash tool](https://code.claude.com/docs/en/sandboxing) is built into Claude Code and restricts Bash commands. Built-in file tools, MCP servers, and hooks still run directly on your host. Every other approach in the table puts the whole Claude Code process inside the isolation boundary, so file tools, MCP servers, and hooks are restricted too.
...
| Work on an untrusted repository                                               | A dedicated virtual machine, or a [cloud session](https://code.claude.com/docs/en/claude-code-on-the-web) if you have a Claude subscription; GitHub is not required when you launch with `claude --cloud` |
...
| Use Claude Code from a device with no local setup                             | A [cloud session](https://code.claude.com/docs/en/claude-code-on-the-web), which requires a Claude subscription and a connected GitHub account                                                            |
