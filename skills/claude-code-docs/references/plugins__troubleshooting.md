...
path: /docs/en/plugins/troubleshooting
---
...
| ` "<name>" has conflicting version requirements`                          | No version satisfies every range that pins it. The message lists the ranges.                      | Uninstall or update one of the conflicting plugins, or ask the upstream author to widen its constraint.                                                                                                                                                             |
...
**Reload if you installed during this session**
...
A plugin's hooks don't run. Either the **Errors** tab shows a load failure for them, the hooks load and you see ` hook error` notices in the transcript, or a hook loads without error and never fires.
