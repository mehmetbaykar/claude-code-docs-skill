...
| `chat:clearInput`     | Ctrl+L                            | Force a full screen redraw, preserving input. In [fullscreen rendering](https://code.claude.com/docs/en/fullscreen#clear-the-conversation), press twice within two seconds to run `/clear` |
...
| `chat:killAgents`     | Ctrl+X Ctrl+K                     | Stop all running [background subagents](https://code.claude.com/docs/en/sub-agents#run-subagents-in-foreground-or-background) in this session                                              |
...
| `confirm:toggleExplanation` | Ctrl+E    | Toggle a model-generated [explanation of the command](https://code.claude.com/docs/en/permissions#permission-system) on Bash and PowerShell permission prompts |
...
| `footer:dismiss`        | Backspace, Delete | Dismiss the selected [artifact](https://code.claude.com/docs/en/artifacts) link from the footer; the published artifact itself is unaffected. On other footer rows, these keys have no effect. Requires v2.1.217 or later |
