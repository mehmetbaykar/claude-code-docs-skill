...
* For every session on the machine: add `"axScreenReader": true` to your user [settings file](https://code.claude.com/docs/en/settings). This covers any terminal, including the VS Code integrated terminal.

The methods are listed in precedence order: the [`--ax-screen-reader`](https://code.claude.com/docs/en/cli-reference#cli-flags) flag overrides the [`CLAUDE_AX_SCREEN_READER`](https://code.claude.com/docs/en/env-vars) environment variable, which overrides the [`axScreenReader`](https://code.claude.com/docs/en/settings#available-settings) setting.
...
Screen reader mode renders as plain scrolling text, even if you've turned on [fullscreen rendering](https://code.claude.com/docs/en/fullscreen) with the [`tui` setting](https://code.claude.com/docs/en/settings#available-settings); the setting has no effect while the mode is active. Attached background sessions still render fullscreen; see [Known limitations](#known-limitations).
...
See the [text editing shortcuts](https://code.claude.com/docs/en/interactive-mode#text-editing) for what each key does.

Cycling [permission modes](https://code.claude.com/docs/en/permission-modes) with `Shift+Tab` announces the mode you land on, such as `[plan mode on]` or `[accept edits on]`. Claude Code prints the announcement once and doesn't repeat it on later redraws. Requires Claude Code v2.1.210 or later.
...
The bell is your terminal's standard alert. To silence it, change the bell setting in your terminal application. The bell doesn't require screen reader mode: outside the mode, set [`preferredNotifChannel`](https://code.claude.com/docs/en/settings#available-settings) to `"terminal_bell"` for similar alerts when Claude is waiting on you. See [Get a terminal bell or notification](https://code.claude.com/docs/en/terminal-config#get-a-terminal-bell-or-notification).
...
* The `CLAUDE_CODE_ACCESSIBILITY` [environment variable](https://code.claude.com/docs/en/env-vars) is for screen magnifiers. Set `CLAUDE_CODE_ACCESSIBILITY=1` to keep the native terminal cursor visible so that magnifiers, such as macOS Zoom, can track the cursor position. The cursor follows keyboard focus: the input caret while you type, and the highlighted row as you move through menus and panels, such as `/config` and `/plugin`, with the arrow keys. Row tracking in menus and panels requires Claude Code v2.1.218 or later.
* The `prefersReducedMotion` [setting](https://code.claude.com/docs/en/settings#available-settings) reduces or disables spinners, shimmer, and other animations without changing the rest of the interface.
* The `theme` [setting](https://code.claude.com/docs/en/settings#available-settings) selects the interface colors, including the colorblind-friendly `dark-daltonized` and `light-daltonized` themes.
...
* [Settings](https://code.claude.com/docs/en/settings#available-settings): the `axScreenReader`, `prefersReducedMotion`, `theme`, and `preferredNotifChannel` entries
* [Environment variables](https://code.claude.com/docs/en/env-vars): the `CLAUDE_AX_SCREEN_READER` and `CLAUDE_CODE_ACCESSIBILITY` entries
* [CLI reference](https://code.claude.com/docs/en/cli-reference#cli-flags): the `--ax-screen-reader` flag
* [Terminal configuration](https://code.claude.com/docs/en/terminal-config): bells, notifications, and themes outside screen reader mode
* [Non-interactive mode](https://code.claude.com/docs/en/headless): scripted `claude -p` runs, which write plain text without screen reader mode
