---
title: "Use Claude Code with a screen reader"
source: https://code.claude.com/docs/en/accessibility
path: /docs/en/accessibility
---
...
* For every session on the machine: add `"axScreenReader": true` to your user [settings file](https://code.claude.com/docs/en/settings). The setting applies in any terminal, including the VS Code integrated terminal.

If you combine methods, Claude Code applies the [`--ax-screen-reader`](https://code.claude.com/docs/en/cli-reference#cli-flags) flag over the [`CLAUDE_AX_SCREEN_READER`](https://code.claude.com/docs/en/env-vars#variables) environment variable, and the variable over the [`axScreenReader`](https://code.claude.com/docs/en/settings-reference#axscreenreader) setting.
...
| :---------------------------------------------------------------------- | :------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`--ax-screen-reader`](https://code.claude.com/docs/en/cli-reference#cli-flags)                     | Flag                 | Screen reader mode for one session.                                                                                                                                                                                                                      |
| [`CLAUDE_AX_SCREEN_READER`](https://code.claude.com/docs/en/env-vars#variables)                     | Environment variable | Screen reader mode for sessions started from the shell where you set it.                                                                                                                                                                                 |
| [`axScreenReader`](https://code.claude.com/docs/en/settings-reference#axscreenreader)               | Setting              | Screen reader mode for every session when `true`.                                                                                                                                                                                                        |
| [`CLAUDE_AX_STARTUP_QUIET_MS`](https://code.claude.com/docs/en/env-vars#variables)                  | Environment variable | How long Claude Code waits after the confirmation line before it draws the first prompt in screen reader mode. Requires Claude Code v2.1.217 or later.                                                                                                   |
| [`CLAUDE_AX_PREPARK_MS`](https://code.claude.com/docs/en/env-vars#variables)                        | Environment variable | How long Claude Code waits, with the cursor at the start of the line, before it writes a new or changed line in screen reader mode. Requires Claude Code v2.1.233 or later.                                                                              |
| [`CLAUDE_CODE_ACCESSIBILITY`](https://code.claude.com/docs/en/env-vars#variables)                   | Environment variable | A terminal cursor that stays visible for screen magnifiers such as macOS Zoom when you set it to `1`. The cursor follows the input caret and, on Claude Code v2.1.218 or later, the highlighted row in menus and panels such as `/config` and `/plugin`. |
| [`prefersReducedMotion`](https://code.claude.com/docs/en/settings-reference#prefersreducedmotion)   | Setting              | Reduced or no spinners, shimmer, and other animations when `true`.                                                                                                                                                                                       |
| [`theme`](https://code.claude.com/docs/en/settings-reference#theme)                                 | Setting              | The interface colors, including the colorblind-friendly `dark-daltonized` and `light-daltonized` themes. You can also pick one with [`/theme`](https://code.claude.com/docs/en/commands#all-commands).                                                                               |
| [`preferredNotifChannel`](https://code.claude.com/docs/en/settings-reference#preferrednotifchannel) | Setting              | With the value `"terminal_bell"`, a terminal bell outside screen reader mode when Claude is waiting on you.                                                                                                                                              |
...
Claude Code leaves everything it prints in your terminal's scrollback, so you can re-read earlier turns with your screen reader's review commands or your terminal's search. Claude Code ignores the [`tui` setting](https://code.claude.com/docs/en/settings-reference#tui) in screen reader mode. Apart from the attached background sessions listed under [Known limitations](#known-limitations), it prints scrolling text instead of [fullscreen rendering](https://code.claude.com/docs/en/fullscreen).

Claude Code also waits at two points so your screen reader can keep up:

* After Claude Code prints the confirmation line, it waits 3 seconds before it draws the prompt, so your screen reader can finish the line. Press any key to end the wait. To change the length of the wait, set [`CLAUDE_AX_STARTUP_QUIET_MS`](https://code.claude.com/docs/en/env-vars#variables).
* Before Claude Code writes a new or changed line, such as a hint or more of Claude's reply, it moves the cursor to the start of the line and waits 50 milliseconds. Your screen reader then reads the line from its first character. Characters you type or delete at the end of the input line appear immediately. To change the length of the wait, set [`CLAUDE_AX_PREPARK_MS`](https://code.claude.com/docs/en/env-vars#variables).
...
| `Cost:`                | The session cost summary when Claude Code exits, if your account [shows costs](https://code.claude.com/docs/en/costs) |
...
When you delete a word or a line with one of the [text editing shortcuts](https://code.claude.com/docs/en/interactive-mode#text-editing), Claude Code announces the deleted text:
...
When you cycle [permission modes](https://code.claude.com/docs/en/permission-modes) with `Shift+Tab`, Claude Code announces the permission mode you land on, such as `[plan mode on]` or `[accept edits on]`. Claude Code prints the announcement once and doesn't repeat it on later redraws.
...
The [`/effort`](https://code.claude.com/docs/en/model-config#adjust-effort-level) selector, which is a slider outside screen reader mode, becomes the same kind of numbered list.
...
The bell is your terminal's standard alert. To silence it, change the bell setting in your terminal application. Outside screen reader mode, set [`preferredNotifChannel`](https://code.claude.com/docs/en/settings-reference#preferrednotifchannel) to `"terminal_bell"` to get a [similar bell](https://code.claude.com/docs/en/terminal-config#get-a-terminal-bell-or-notification) when Claude is waiting on you.
...
* Claude Code doesn't announce a permission mode change made in any way other than cycling with `Shift+Tab`, such as entering [plan mode](https://code.claude.com/docs/en/permission-modes#analyze-before-you-edit-with-plan-mode) from a command.
* Attaching to a [background session](https://code.claude.com/docs/en/agent-view) with `claude attach` or from agent view enters the terminal's alternate screen, which has no native scrollback. This is the [same behavior as other attached sessions](https://code.claude.com/docs/en/fullscreen). To get back out, press Left Arrow on an empty prompt, or Ctrl+Z if a dialog has focus.
* Claude Code announces costs in the summary it prints at exit, not per turn.
* Screen reader mode doesn't change [non-interactive mode](https://code.claude.com/docs/en/headless) with the `-p` flag. Non-interactive mode already writes plain text and remains an alternative for scripting.
