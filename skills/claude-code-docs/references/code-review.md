...
Because it's pasted verbatim, `REVIEW.md` is plain instructions: [`@` import syntax](https://code.claude.com/docs/en/memory#import-additional-files) is not expanded, and referenced files are not read into the prompt. Put the rules you want enforced directly in the file.
...
* `--post`: on an `ultra` cloud review of a `github.com` pull request, posts the finished findings to the PR as one comment from your GitHub account; see [Post findings to the pull request](https://code.claude.com/docs/en/ultrareview#post-findings-to-the-pull-request). Requires Claude Code v2.1.227 or later
