...
* [`effortLevel`](https://code.claude.com/docs/en/settings-reference#effortlevel): use [`/effort`](https://code.claude.com/docs/en/model-config#adjust-effort-level) to change it mid-session
...
* **Managed settings**: only [server-managed settings](https://code.claude.com/docs/en/server-managed-settings) reach a cloud session; a `managed-settings.json` file or MDM profile on your device doesn't. A [self-hosted environment](https://code.claude.com/docs/en/self-hosted-environments) reads the managed settings file in its runner image only when server-managed settings deliver no keys, apart from the [keys Claude Code reads from every admin source](https://code.claude.com/docs/en/managed-settings#keys-read-from-every-admin-source); see [settings precedence](https://code.claude.com/docs/en/server-managed-settings#settings-precedence).
