...
Learn more: [How Claude Code works](https://code.claude.com/docs/en/how-claude-code-works#the-agentic-loop)
...
Learn more: [Auto memory](https://code.claude.com/docs/en/memory#auto-memory)

### Auto mode

A [permission mode](#permission-mode) where a separate classifier model reviews actions instead of you, so Claude Code runs most of them without asking you. Claude Code still asks you before actions your explicit ask rules match. On Pro, Max, and Team plans, auto mode is the [built-in starting permission mode](https://code.claude.com/docs/en/permission-modes#which-mode-a-session-starts-in) for interactive terminal and VS Code sessions. The classifier blocks scope escalation, untrusted infrastructure, and [prompt injection](#prompt-injection). Tool results are stripped from what it sees, so hostile content in a file or web page can't manipulate it directly.

Learn more: [Eliminate prompts with auto mode](https://code.claude.com/docs/en/permission-modes#eliminate-prompts-with-auto-mode)
...
Learn more: [Bundled skills](https://code.claude.com/docs/en/skills#bundled-skills)
...
Learn more: [The `.claude` directory](https://code.claude.com/docs/en/claude-directory)
...
Learn more: [CLAUDE.md files](https://code.claude.com/docs/en/memory#claude-md-files)

### Cloud session

A Claude Code session that keeps running after you close your laptop, because it runs on cloud infrastructure instead of your machine: Anthropic-managed by default, or a [self-hosted environment](https://code.claude.com/docs/en/self-hosted-environments) your organization operates. You start one from claude.ai/code, the Claude mobile app, the Desktop app with **Cloud** selected, `claude --cloud`, or a [routine](https://code.claude.com/docs/en/routines). A session in your terminal, IDE, or the Desktop app with **Local** selected is a local session; to reach a local session from another device, use [Remote Control](#remote-control).

Learn more: [Use Claude Code in the cloud](https://code.claude.com/docs/en/claude-code-on-the-web)
...
Two other uses of the word are unrelated: `claude` CLI subcommands such as `claude mcp add`, listed in the [CLI reference](https://code.claude.com/docs/en/cli-reference#cli-commands), and the `command` field of a stdio [MCP server](#mcp-server) entry, which specifies the executable Claude Code launches to start the server.
...
Learn more: [What survives compaction](https://code.claude.com/docs/en/context-window#what-survives-compaction) · [When context fills up](https://code.claude.com/docs/en/how-claude-code-works#when-context-fills-up)
...
Learn more: [Use MCP servers from claude.ai](https://code.claude.com/docs/en/mcp#use-mcp-servers-from-claude-ai)
...
Learn more: [Explore the context window](https://code.claude.com/docs/en/context-window)
...
Learn more: [Sessions from Dispatch](https://code.claude.com/docs/en/desktop#sessions-from-dispatch)
...
Learn more: [Adjust effort level](https://code.claude.com/docs/en/model-config#adjust-effort-level)
...
Learn more: [Use extended thinking](https://code.claude.com/docs/en/model-config#extended-thinking)
...
Learn more: [Skill frontmatter](https://code.claude.com/docs/en/skills#frontmatter-reference), [Subagent frontmatter](https://code.claude.com/docs/en/sub-agents#supported-frontmatter-fields), [Output style frontmatter](https://code.claude.com/docs/en/output-styles#frontmatter), [Rule frontmatter](https://code.claude.com/docs/en/memory#rules-frontmatter-reference)
...
Learn more: [Server-managed settings](https://code.claude.com/docs/en/server-managed-settings) · [Settings files](https://code.claude.com/docs/en/settings#where-settings-live)
...
Learn more: [Model Context Protocol](https://code.claude.com/docs/en/mcp)
...
Learn more: [Scale with MCP Tool Search](https://code.claude.com/docs/en/mcp#scale-with-mcp-tool-search)

## N

### Non-interactive mode

A mode that executes a single prompt and exits without an interactive prompt, invoked with `-p` or `--print`. Used for CI, scripts, and piping. The run is still saved as a resumable session unless you pass `--no-session-persistence`. The [Agent SDK](https://code.claude.com/docs/en/agent-sdk/overview) is the Python and TypeScript equivalent. Formerly called headless mode.

Learn more: [Run Claude Code programmatically](https://code.claude.com/docs/en/headless)
...
Learn more: [Choose a permission mode](https://code.claude.com/docs/en/permission-modes)
...
Learn more: [Configure permissions](https://code.claude.com/docs/en/permissions)
...
Learn more: [Analyze before you edit with plan mode](https://code.claude.com/docs/en/permission-modes#analyze-before-you-edit-with-plan-mode)
...
Learn more: [Protect against prompt injection](https://code.claude.com/docs/en/security#protect-against-prompt-injection)

## R

### Remote Control

A way to continue a local Claude Code session from your phone or browser via claude.ai. Your code execution and files stay on your machine; the interface is remote. Different from a [cloud session](https://code.claude.com/docs/en/claude-code-on-the-web), which runs in a cloud sandbox.
...
Learn more: [Extend Claude with skills](https://code.claude.com/docs/en/skills)

### Subagent

A specialized AI assistant that runs in its own context window with a custom system prompt, specific tool access, and independent permissions. It works on a delegated task and returns a summary to the main conversation. Use subagents to keep large explorations out of your primary context or to run parallel research. A subagent stays inside the session that spawned it. To pass findings between separate sessions you run yourself, use [cross-session messaging](https://code.claude.com/docs/en/cross-session-messaging).

Built-in subagents include Explore, Plan, and general-purpose.

Learn more: [Create custom subagents](https://code.claude.com/docs/en/sub-agents)

### Surface

Any place you access Claude Code: the CLI, VS Code, JetBrains, Desktop, or claude.ai. All surfaces share the same engine. Sessions on your machine read your local CLAUDE.md, settings, and skills; [cloud sessions](https://code.claude.com/docs/en/cloud-environments#what-carries-over-from-your-setup) start from a fresh clone of your repository and don't read `~/.claude/` on your machine. Slack and the Chrome extension are integrations that connect to a surface rather than surfaces themselves.

Learn more: [Platforms and integrations](https://code.claude.com/docs/en/platforms)
...
Learn more: [Tools available to Claude](https://code.claude.com/docs/en/tools-reference)
...
Learn more: [Give Claude a way to verify its work](https://code.claude.com/docs/en/best-practices#give-claude-a-way-to-verify-its-work)
...
Learn more: [Run parallel sessions with git worktrees](https://code.claude.com/docs/en/worktrees)
