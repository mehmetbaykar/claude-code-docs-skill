...
| **Plugin servers only** | Servers can only come from plugins; users can't add their own                                                         | [`strictPluginOnlyCustomization`](https://code.claude.com/docs/en/settings#strictpluginonlycustomization) with `mcp` in the list |
...
See [Invalid entries in managed settings](https://code.claude.com/docs/en/settings#invalid-entries-in-managed-settings) for what happens when an entry fails schema validation.
...
| `allowedMcpServers`          | Allowlist of permitted servers                                                                                                                                                                          | Any [settings file](https://code.claude.com/docs/en/settings#settings-files); entries from every source merge unless `allowManagedMcpServersOnly` is set | For enforcement, a [managed settings source](https://code.claude.com/docs/en/admin-setup#decide-how-settings-reach-devices): server-managed settings, `managed-settings.json`, MDM profile, or registry |
