...
`managed-mcp.json` is a standalone file, so it cannot be delivered through [server-managed settings](https://code.claude.com/docs/en/server-managed-settings). Any process that can write to a system path with administrator privileges can deploy it. At scale, that's usually through device management tooling, such as Jamf or a configuration profile on macOS, Group Policy or Intune on Windows, or your fleet management of choice on Linux. Claude Code looks for the file at one of these paths:
...
Before loading a server, including one from `managed-mcp.json`, Claude Code runs the three checks below in order. In-process `type: "sdk"` servers, which the [app that started the session registers](https://code.claude.com/docs/en/mcp#how-connectors-reach-claude-code), skip all three.
