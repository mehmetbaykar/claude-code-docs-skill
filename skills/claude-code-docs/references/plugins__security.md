---
title: "Plugin security and trust"
source: https://code.claude.com/docs/en/plugins/security
path: /docs/en/plugins/security
---
...
* **Claude Code's own security model**: see [Security](https://code.claude.com/docs/en/security)
* **Restricting or requiring plugins for an organization**: see [Manage plugins for your organization](https://code.claude.com/docs/en/plugins/org)
* **The `security-guidance` or `claude-security` plugins**: this page isn't about them. See [`security-guidance`](https://code.claude.com/docs/en/security-guidance) and [`claude-security`](https://code.claude.com/docs/en/claude-security)
...
* **Hooks**: a plugin's [hooks](https://code.claude.com/docs/en/hooks) run as shell commands at points in Claude Code's lifecycle, such as before or after a tool call.
* **MCP and LSP servers**: Claude Code connects to the [MCP servers](https://code.claude.com/docs/en/mcp) an enabled plugin declares and gives Claude their tools. A stdio MCP server runs as a process that Claude Code starts on your machine. Claude Code also starts the language servers the plugin declares.
...
* **Updates**: when auto-update is on for the marketplace you installed a plugin from, Claude Code updates that plugin in the background, so the files you reviewed can change on disk. [When auto-update runs](https://code.claude.com/docs/en/plugins/loading#when-auto-update-runs) has the timing. To turn auto-update on or off per marketplace, see [Keep plugins updated](https://code.claude.com/docs/en/plugins/install#keep-plugins-updated).

Claude Code's [permission rules](https://code.claude.com/docs/en/permissions) and [sandbox](https://code.claude.com/docs/en/sandboxing) cover the tool calls Claude makes, not the code a plugin runs by itself:
...
Installing a plugin also enables it, unless its manifest or marketplace entry sets [`defaultEnabled: false`](https://code.claude.com/docs/en/plugins/install#choose-an-install-scope) and you haven't enabled it yourself.
...
For how the official, community, and demo marketplaces differ and where to browse what each one lists, see [Anthropic's marketplaces](https://code.claude.com/docs/en/plugins/anthropic-marketplaces).
...
**Check the marketplace's source**
...
In your shell, run [`claude plugin uninstall <plugin>`](https://code.claude.com/docs/en/plugins/cli-reference#plugin-uninstall) with the `--scope` you installed it at. Then check what the uninstall removed and what it left:
...
* **Cached files**: the plugin's files stay on disk under `~/.claude/plugins/cache/` for 14 days before a [background sweep removes them](https://code.claude.com/docs/en/plugins/loading#cleanup-of-previous-versions). After you uninstall your last plugin, orphaned directories stay until you install another. To delete the files now, remove the plugin's directory under `~/.claude/plugins/cache/<marketplace>/<plugin>/` yourself.
* **The marketplace**: if you don't trust the marketplace's owner either, [remove the marketplace](https://code.claude.com/docs/en/plugins/install#manage-marketplaces) too, which uninstalls every plugin you installed from it.
...
If your organization sets `pluginTrustMessage` in [managed settings](https://code.claude.com/docs/en/plugins/org), Claude Code appends that text to the warning.
...
* **Untrusted marketplace source**: when a marketplace uses an official or community name but its source is outside `github.com/anthropics/`, Claude Code stops loading the marketplace and the plugins you installed from it. The error is [Marketplace is registered from an untrusted source](https://code.claude.com/docs/en/errors#marketplace-is-registered-from-an-untrusted-source).
* **Archive integrity**: when a marketplace entry pins an [`archive` source](https://code.claude.com/docs/en/plugins/marketplace-reference#archive-plugin-source) to a `sha256` digest and the downloaded file's digest doesn't match it, Claude Code refuses the install. The error is [Plugin archive integrity check failed](https://code.claude.com/docs/en/errors#plugin-archive-integrity-check-failed).
...
With [managed settings](https://code.claude.com/docs/en/plugins/org), an administrator can enforce these plugin controls:
...
* Stop plugins from members' claude.ai accounts from loading in Claude Code, with [`syncClaudeAiPlugins`](https://code.claude.com/docs/en/plugins/org#control-matrix)

The [control matrix](https://code.claude.com/docs/en/plugins/org#control-matrix) says what each key does and doesn't cover.

## Find plugins in telemetry

If your organization exports Claude Code's [OpenTelemetry events](https://code.claude.com/docs/en/monitoring-usage) to its own backend, the [marketplace tiers](#marketplace-tiers) decide which plugin names appear there:

* **[Plugin loaded event](https://code.claude.com/docs/en/monitoring-usage#plugin-loaded-event)**: the event reports official-tier plugin and marketplace names as they are. For the community and third-party tiers, `plugin.name` and `marketplace.name` are the literal string `third-party` unless you set `OTEL_LOG_TOOL_DETAILS=1`.
* **Plugin scope**: the loaded event's `plugin.scope` still reports where the plugin came from, such as `org` for a plugin your managed settings enable or `user-local` for any other third-party plugin. The [plugin loaded event](https://code.claude.com/docs/en/monitoring-usage#plugin-loaded-event) lists every value.
* **[Plugin installed event](https://code.claude.com/docs/en/monitoring-usage#plugin-installed-event)**: unless you set `OTEL_LOG_TOOL_DETAILS=1`, the event omits the name fields for non-official plugins instead of reporting `third-party`.
...
* [Manage plugins for your organization](https://code.claude.com/docs/en/plugins/org): restrict which marketplaces users can install from and require the ones you trust
* [Install and manage plugins](https://code.claude.com/docs/en/plugins/install): review a plugin's details pane before you choose a scope
* [Anthropic's marketplaces](https://code.claude.com/docs/en/plugins/anthropic-marketplaces): which marketplace names are Anthropic's
* [Security](https://code.claude.com/docs/en/security): Claude Code's own security model
